...
(() => {
  ...
  async function captureTranscript() {
    ...
    const displayedTranscript = extractor.isDisplayedTranscriptPanel(transcriptPanel);
    const languageSelection = displayedTranscript
      ? {
    ...
    const cueReadStartedAt = Date.now();
    const cueReadTimeoutMs = displayedTranscript ? 12_000 : 90_000;
    while (!finished && Date.now() - cueReadStartedAt < cueReadTimeoutMs) {
      const panel = panelOpener.findTranscriptPanel(document) || transcriptPanel;
      if (displayedTranscript && panel !== transcriptPanel) {
    ...
    if (!finished) {
      if (displayedTranscript) {
        const panel = panelOpener.findTranscriptPanel(document) || transcriptPanel;
        const fallbackText = String(panel?.innerText ?? '').trim();
        if (fallbackText && fallbackText.length <= 20_000) {
          const heading = document.querySelector('h1.ytd-watch-metadata yt-formatted-string, ytd-watch-metadata h1, h1');
          const title = visibleText(heading) || document.title.replace(/\s+-\s+YouTube\s*$/i, '').trim();
          const message = {
            type: 'captiongrab.transcript-fallback',
            requestID,
            videoID: requestedVideoID,
            videoURL: `https://www.youtube.com/watch?v=${requestedVideoID}`,
            title,
            captionLanguage: 'as-displayed',
            isAutoGenerated: null,
            fallbackText,
            progress: panelResult.progress,
            debugLog: appendDebugEvent(debugLog, 'sending visible text for on-device transcript fallback', {
              sourceCharacterCount: fallbackText.length
            })
          };
          finished = true;
          sendToApp(message);
      ...
      sendError(
        ...
        appendDebugEvent(debugLog, 'transcript read timed out', { timeoutMs: cueReadTimeoutMs, language: languageSelection.selectedLanguage })
      );
    }
  }
