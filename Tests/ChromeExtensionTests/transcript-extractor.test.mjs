...
test('captures the shown transcript in YouTube’s searchable transcript panel without a language menu', async () => {
  const fixture = languageMenuFixture([], '');
  fixture.document.children = fixture.document.children.filter(child => child !== fixture.panel);
  fixture.panel.children = [];
  fixture.panel.innerText = '';
  const engagementPanel = fixture.document.append(new FakeDOMNode({
    tag: 'ytd-engagement-panel-section-list-renderer',
    attributes: { 'target-id': 'engagement-panel-searchable-transcript' }
  }));
  engagementPanel.append(fixture.panel);
  const row = fixture.panel.append(new FakeDOMNode({ tag: 'ytd-transcript-segment-renderer' }));
  row.querySelector = selector => selector.includes('timestamp')
    ? element('0:00')
    : selector.includes('text') ? element('A short displayed English caption.') : null;

  const message = await runContentCapture(fixture);
  assert.equal(message.type, 'captiongrab.transcript');
  assert.equal(message.captionLanguage, 'as-displayed');
  assert.equal(message.isAutoGenerated, null);
  assert.deepEqual(message.cues, [{ startTimeMilliseconds: 0, text: 'A short displayed English caption.' }]);
  assert.match(message.debugLog, /displayed-transcript/);
  assert.ok(!message.debugLog.includes('A short displayed English caption.'));
  assert.equal(fixture.trigger.clickCount, 0);
});
...
test('sends visible transcript text for on-device recovery when the displayed panel has unknown rows', async () => {
  const fixture = languageMenuFixture([], '');
  fixture.panel.children = [];
  fixture.panel.attributes['target-id'] = 'PAmodern_transcript_view';
  fixture.panel.innerText = 'Transcript\n0:00\nA synthetic caption from an unfamiliar layout.';

  const message = await runContentCapture(fixture);

  assert.equal(message.type, 'captiongrab.transcript-fallback');
  assert.equal(message.captionLanguage, 'as-displayed');
  assert.equal(message.fallbackText, fixture.panel.innerText);
  assert.ok(!message.debugLog.includes('A synthetic caption from an unfamiliar layout.'));
  assert.match(message.debugLog, /on-device transcript fallback/);
});
...
test('extension manifest requests only the YouTube page and native-messaging permission', async () => {
  ...
  const serviceWorkerSource = await readFile(path.join(root, 'ChromeExtension/service-worker.js'), 'utf8');
  assert.ok(panelOpenerSource.includes('Show transcript'));
  assert.ok(serviceWorkerSource.includes('captiongrab.transcript-fallback'));
