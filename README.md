...
5. After installing CaptionGrab 1.2.11, open `chrome://extensions` and click **Reload** on the CaptionGrab unpacked extension. Confirm Chrome shows extension version **1.2.9**. If Reload reports a missing folder, remove only the stale CaptionGrab extension and use **Load unpacked** to select the `ChromeExtension` folder revealed by **Set up Chrome extension**.
