...
4. Click **Get transcript** as usual. When direct retrieval is blocked, CaptionGrab opens the matching video in Chrome, expands the description, waits for the Transcript section, clicks **Show transcript**, selects the Transcript tab if the panel defaults to Chapters, and waits for transcript rows before reading.
5. After installing CaptionGrab 1.2.3, open `chrome://extensions` and click **Reload** on the CaptionGrab unpacked extension so Chrome uses the updated automation. You do not need to remove and re-add it; its extension ID stays the same.
6. If automatic panel opening fails, CaptionGrab displays a message telling you to expand “…more” if shown, scroll to and click **Show transcript**, click the **Transcript** tab if the **In this video** panel opens on Chapters, wait for transcript lines, and retry. The error also shows exactly which automation steps succeeded: description expanded, Transcript section found, button found/clicked, panel opened, tab selected, and lines loaded. Other setup failures show the exact operation, path, OS error domain/code, and underlying error text.
