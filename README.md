...
5. After installing CaptionGrab 1.2.4, open `chrome://extensions` and click **Reload** on the CaptionGrab unpacked extension so Chrome uses the updated automation. You do not need to remove and re-add it; its extension ID stays the same.
6. If automatic panel opening fails, click **Copy debug log** beneath CaptionGrab’s message, then paste the copied text into your reply. The log records relative timings, transcript-related button labels/text and visibility, observed open panels, and controls clicked or attempted. It does not include caption text. Other setup failures show the exact operation, path, OS error domain/code, and underlying error text.
