    ...
    static func fetch(
        ...
        while ContinuousClock.now < deadline {
            ...
            if let data = try? Data(contentsOf: resultURL) {
                ...
                if message.type == "captiongrab.transcript-fallback" {
                    let input = try message.makeOnDeviceFallbackInput(
                        expectedRequestID: requestID,
                        expectedVideoID: link.videoID,
                        canonicalURL: link.canonicalURL
                    )
                    let cues = try await OnDeviceTranscriptFallback.recover(sourceText: input.sourceText)
                    return TranscriptData(
                        videoTitle: input.title,
                        videoURL: link.canonicalURL,
                        isAutoGenerated: nil,
                        cues: cues,
                        recoveredWithOnDeviceAI: true
                    )
                }
                do {
